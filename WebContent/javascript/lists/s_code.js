...
function s_doPlugins(s) {
  ...
  if (_full) {
    s.pageName=s.pageName+":"+mtx_fn_path(_path);
  }
  ...
  if( s_account=="tivirgilioglobalprod,tivirmailservprod" ) {
    ...
    if( s.pageName=='VIR:Virgilio Mail:Inserimento dati' ) {
      s.eVar61=s.getQueryParam('entryPoint');
    }
  }

  /* Impostazione cookie profilazione. 
     Adobe Consulting gsessa: Allocation of Prop31 should be commented/removed whenever new vista rule is in place */ 
  if(!s.eVar33) s.eVar33='D=np_pt';
  if(!s.prop31) s.prop31='D=np_pt';
  ...
  if(s.prop32&&!s.eVar32) s.eVar32='D=c32';
	...
	if (s.prop36){
		  s.prop33=s.getPercentPageViewed();
       }
  
  //Impostazione canale precedente
  s.eVar37=s.getPreviousValue(s.channel,"s_pch");
   
  //Reperimento Codici ISTAT per LOCAL -- Commentato in data 26/10/2011 - Giacomo Filidei
  //if ((s.prop9)&&(window.LOCAL_COMMON))
  //{if ((s.prop9.toLowerCase()=="local") && (LOCAL_COMMON.idCity)) {
  ...
  /* Tracciamento cookie np_id */
  if(!s.prop34) {
    var np_idCookie=getOmnitureCookie("np_id");
    var np_idCookieFields=np_idCookie.split("|",4);
    var np_idCookieVersion=np_idCookieFields.shift();
    /* Considero solo la versione 2.1 del cookie */
    if(np_idCookieVersion=="2.1") {
      s.prop34=np_idCookieFields.join("|");
  ...
  if(s.channel) {
    ...
    if(!s.prop14) {
      if(s.prop9.toLowerCase()=="local" && s_dlh != s_1254)
        s.prop14="Local Senza 1254";
    }
    //Aggregato 4
  ...
  if(!s.prop13) {
    if(s.prop2&&s.prop2.toLowerCase()=="profili football")
      s.prop13="Football";
  }
  ...
  /* Internal Search */
	...
	if(t_search) 
    s.events=s.apl(s.events,s.successfulSearchEvent,',',1);
